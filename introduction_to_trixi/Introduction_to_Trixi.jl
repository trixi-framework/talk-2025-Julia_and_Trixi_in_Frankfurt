...
# ╔═╡ 208484e0-07fd-4e13-b046-39bea16d9b9b
using Trixi

# ╔═╡ b2d22b03-00d0-4c6c-bec8-c747835187eb
begin
	using Plots
...
# ╔═╡ e97e43e1-1035-4501-a23b-ade6ef3b0931
begin
	# PassiveTracerEquations setup for Kelvin-Helmholtz instability
	using Trixi: AbstractEquations, rotate_to_x
	import Trixi: nvariables, varnames, flux,
	              cons2prim, prim2cons, density, pressure, density_pressure,
	              max_abs_speeds, max_abs_speed_naive, entropy, cons2entropy
	
	struct PassiveTracerEquations{NDIMS, NVARS, NTRACERS,
	                              FlowEquations <: AbstractEquations{NDIMS}} <: AbstractEquations{NDIMS, NVARS}
	    flow_equations::FlowEquations
	    # TODO: Ensure that the number of variables fits
	end
	
	function PassiveTracerEquations(flow_equations::AbstractEquations, ::Val{NTRACERS}) where {NTRACERS}
	    NVARS = nvariables(flow_equations) + NTRACERS
	    return PassiveTracerEquations{ndims(flow_equations), NVARS, NTRACERS, typeof(flow_equations)}(flow_equations)
	end
	
	@inline ntracers(::PassiveTracerEquations{NDIMS, NVARS, NTRACERS}) where {NDIMS, NVARS, NTRACERS} = NTRACERS
	...
	@inline function flow_variables(u, tracer_equations::PassiveTracerEquations)
	    ...
	    return SVector(ntuple(i -> u[i], Val(n_flow_variables))...)
	end
	
	@inline function tracers(u, tracer_equations::PassiveTracerEquations)
	    n_flow_variables = nvariables_flow(tracer_equations)
	    n_tracers = ntracers(tracer_equations)
	
	    rho = density(u, tracer_equations)
	    return SVector(ntuple(i -> u[n_flow_variables + i] / rho, Val(n_tracers))...)
	end
	
	@inline function cons2prim(u, tracer_equations::PassiveTracerEquations)
	    (; flow_equations) = tracer_equations
	    return SVector(cons2prim(flow_variables(u, tracer_equations), flow_equations)...,
	...
	@inline function prim2cons(u, tracer_equations::PassiveTracerEquations)
	    ...
	    return SVector(prim2cons(u_flow, flow_equations)...,
	                    (rho * u[n_flow_variables + i] for i=1:n_tracers)...)
	end
	
	for f in [:density, :pressure, :density_pressure, :max_abs_speeds]
	    @eval @inline function $f(u, tracer_equations::PassiveTracerEquations)
	        (; flow_equations) = tracer_equations
	        u_flow = flow_variables(u, tracer_equations)
	        return $f(u_flow, flow_equations)
	    end
	end
	...
	@inline function max_abs_speed_naive(u_ll, u_rr, orientation_or_normal,
	    ...
	    return max_abs_speed_naive(u_flow_ll, u_flow_rr, orientation_or_normal, flow_equations)
	end
	...
	@inline function entropy(cons, tracer_equations::PassiveTracerEquations)
	    ...
	    tracer_entropy = 0.5f0 * density(cons, tracer_equations) * sum(abs2, tracers(cons, tracer_equations))
	...
	@inline function cons2entropy(u, tracer_equations::PassiveTracerEquations)
	    ...
	    variables = SVector(flow_entropy[1] - 0.5f0 * sum(abs2, tracers_),
	                        (flow_entropy[i] for i=2:nvariables(flow_equations))...,
	                        tracers_...)
	    return variables
	end
	...
	@inline function flux(u, orientation::Integer,
	    ...
	    # Awkward way to fix box type instability when using `if`
	    # TODO: We should fix this when moving it to Trixi.jl.
	    #       Ideally, we could have an interface `velocity(u, equations)`
	    #       that we just need to use here.
	    v_normal = u[1 + orientation] / density(u, tracer_equations)
	
	    flux_tracer = SVector(ntuple(@inline(v->u[v+n_flow_variables]*v_normal),
	                          Val(ntracers(tracer_equations))))
	
	    flux_flow = flux(u_flow, orientation, tracer_equations.flow_equations)
	...
	@inline function flux(u, normal_direction::AbstractVector,
	    ...
	    #       Ideally, we could have an interface `velocity(u, equations)`
	    #       that we just need to use here.
	    v_normal = (normal_direction[1] * u[2] + normal_direction[2] * u[3]) / density(u, tracer_equations)
	
	    flux_tracer = SVector(ntuple(@inline(v->u[v+n_flow_variables]*v_normal),
	                          Val(ntracers(tracer_equations))))
	
	    flux_flow = flux(u_flow, normal_direction, tracer_equations.flow_equations)
	...
	@inline function (f::FluxTracerEquationsCentral)(u_ll, u_rr,
	   ...
	   rho_ll = density(u_ll, flow_equations)
	   rho_rr = density(u_rr, flow_equations)
	   flux_tracer = SVector(ntuple(@inline(v -> u_ll[v + n_flow_variables] / rho_ll
	                                           + u_rr[v + n_flow_variables] / rho_rr),
	                         Val(ntracers(tracer_equations))))
	...
	@inline function boundary_condition_slip_wall(u_inner, orientation::Int,
	                                              direction, x, t,
	...
	@inline function boundary_condition_slip_wall(u_inner, normal_direction::AbstractVector,
	                                              ...
	                                              surface_flux_function,
	                                              tracer_equations::PassiveTracerEquations)
	    flow_equations = tracer_equations.flow_equations
	    norm_ = norm(normal_direction)
	    # Normalize the vector without using `normalize` since we need to multiply by the `norm_` later
	    normal = normal_direction / norm_
	
	    # rotate the internal solution state
	    u_inner_flow = flow_variables(u_inner, tracer_equations)
	    u_local = rotate_to_x(u_inner_flow, normal, flow_equations)
	
	    # compute the primitive variables
	    rho_local, v_normal, v_tangent, p_local = cons2prim(u_local, flow_equations)
	    ...
	    if v_normal <= 0.0
	        sound_speed = sqrt(flow_equations.gamma * p_local / rho_local) # local sound speed
	        p_star = p_local *
	                 (1 + 0.5 * (flow_equations.gamma - 1) * v_normal / sound_speed)^(2 *
	                                                                             flow_equations.gamma *
	                                                                             flow_equations.inv_gamma_minus_one)
	    else # v_normal > 0.0
	        A = 2 / ((flow_equations.gamma + 1) * rho_local)
	        B = p_local * (flow_equations.gamma - 1) / (flow_equations.gamma + 1)
	        p_star = p_local +
	                 0.5 * v_normal / A *
	    ...
	    return SVector(zero(eltype(u_inner_flow)),
	                   p_star * normal[1],
	                   p_star * normal[2],
	                   zero(eltype(u_inner_flow)),
	                   v_normal * u_inner[5] / u_inner[1]) * norm_
	end	
	# End of passive tracer extension
	###############################################################################
	...
	function cons2density_and_tracers(u, tracer_equations::PassiveTracerEquations)
	    return SVector(density(u, tracer_equations), tracers(u, tracer_equations)...)
	end
	
	function varnames(variables::typeof(cons2density_and_tracers),
	                  tracer_equations::PassiveTracerEquations)
	    n_tracers = ntracers(tracer_equations)
	    return ("rho", ntuple(i -> "xi_$i", Val(n_tracers))...)
	end
end
...
# ╔═╡ aec3ad12-4f29-4f55-a8b0-c4c822cae861
md"""
...
$$\frac{\partial}{\partial \tau} \begin{pmatrix}
\phi \\
q_1 \\
q_2
\end{pmatrix} +
...
# ╔═╡ d6a19f80-722e-43d3-b92f-03d64dd13020
html"""
<div style="display: flex; align-items: center; gap: 20px;">
  <img width="400" src="https://raw.githubusercontent.com/trixi-framework/paper-self-gravitating-gas-dynamics/ede05f80c5cf97a513d7fd0c5bc07c639b05abe5/assets/sedov-rho-phi-mesh.gif">
  <p style="font-size: 1.2rem; max-width: 300px;">
Schlottke-Lakemper, Winters, Ranocha, Gassner <br> <br>
Journal of Computational Physics, 2021
  </p>
</div>
"""

# ╔═╡ 817e6005-f7a2-4f7e-aafb-b1d675e77aaa
html"""
<h1 class="custom-heading"><center>Trixi.jl in action <br>  
...
# ╔═╡ 85334a71-6c28-4dd0-a981-bedff47f3926
html"""
<center>
<img width = 1200 src="https://gist.github.com/Arpit-Babbar/6f8e445d9126f41aa96c9e2b5fb0bc99/raw/1396e4a2aadb9bb9f056f034a05f8477fb81ef6f/Pasted%2520image%252020250112194941.png">
</center>
"""

# ╔═╡ e25382ab-862b-4b0a-8d61-56678c946580
html"""
<center>
<font size = 3>
Doehring, Schlottke-Lakemper, Gassner, Manuel Torrilhon <br>
Journal of Computational Physics, 2024
</font>
</center>
"""

# ╔═╡ d12849d0-c94b-4d79-8c94-82718d760af2
html"""
<h1 class="custom-heading"><center>Trixi.jl in action <br> Baroclinic instability
</center></h1>
"""

# ╔═╡ 453a5e2d-ee88-490b-8e49-dc76c0ed672e
html"""
<center>
<img width = 600 src="https://gist.githubusercontent.com/Arpit-Babbar/6f8e445d9126f41aa96c9e2b5fb0bc99/raw/d1a644ae8579cdf78a67ed5730fac5e1e82a31da/baroclinic_1.png">
</center>
"""

# ╔═╡ 4e1c5ba8-bfa1-4824-8eac-72f4a6d7187e
html"""
<center>
<font size = 3>
Cubed-sphere mesh and pressure contours projected on a picture of Earth
</font>
</center>
"""

# ╔═╡ 87ef0a5e-883f-4e71-a6c9-b053b836cf9b
html"""
<div style="display: flex; align-items: center; gap: 20px;">
<img width = 330 src="https://gist.githubusercontent.com/Arpit-Babbar/6f8e445d9126f41aa96c9e2b5fb0bc99/raw/d1a644ae8579cdf78a67ed5730fac5e1e82a31da/baroclinic_3.png">
<img width = 330 src="https://gist.githubusercontent.com/Arpit-Babbar/6f8e445d9126f41aa96c9e2b5fb0bc99/raw/d1a644ae8579cdf78a67ed5730fac5e1e82a31da/baroclinic_2.png">
</div>
"""

# ╔═╡ 4e5fccbd-9aeb-4897-abbd-fb4870b8b1d8
html"""
<center>
<font size = 3>
Surface pressure with Trixi.jl (left) and reference solution (MCore; right). <br> <b>Credit: Erik Faulhaber et al.</b>
</font>
</center>
"""

# ╔═╡ 0b2cb17d-987e-4c4d-8bdf-a39a673128dd
html"""
<h1 class="custom-heading"><center>How Trixi.jl works<br> 
Discontinuous Galerkin</center></h1>
"""

# ╔═╡ a1a23f17-74ff-4887-a45d-c08595eaa545
md"""
$$\Huge \boldsymbol{u}_t + \boldsymbol{f}(\boldsymbol{u})_x + \boldsymbol{g}(\boldsymbol{u})_y = 0$$
"""
...
html"""
<h1 style="font-size:3.0vw"><center>How Trixi.jl works <br>
High Level
</center></h1>
"""

# ╔═╡ f722e891-d638-4912-85e7-169797acc89e
html"""
<center>
<img width = 1200 src="https://raw.githubusercontent.com/trixi-framework/talk-2021-Introduction_to_Julia_and_Trixi/main/media/components.png">
</center>
"""

# ╔═╡ 6accb70a-f75b-4514-88f0-a20831dc8c5d
html"""
<h1 class="custom-heading"><center>How Trixi.jl works<br> 
An "elixir" file</center></h1>
"""

# ╔═╡ 8c0868df-8a53-444b-b6eb-b1d1b483b917
md"""
```
using OrdinaryDiffEq
using Trixi
...
# ╔═╡ 32364a68-0cd4-4300-a066-4dc361af0c58
html"""<h1><center> Specify equations </center> </h1> """

# ╔═╡ 4a60f39f-d642-4190-9673-4b91d2753f33
md"""
$$\Huge u_t + a u_x + b u_y = 0$$
"""

# ╔═╡ a5b946a9-e810-4dc9-b81f-a567a1c4d415
begin
...
# ╔═╡ 0d429f79-1c90-4421-833c-bf6264892db3
html"""<h1><center>Discontinuous Galerkin solver</h1></center>"""

# ╔═╡ fe56d72d-d9b1-412b-b426-2c1767d99917
begin
	# Create DG solver with polynomial degree = 3 and (local) Lax-Friedrichs/Rusanov flux as surface flux
	solver = DGSEM(polydeg = 3, surface_flux = flux_lax_friedrichs)
end

# ╔═╡ 83409f24-a469-481a-901b-8f9e6efd445c
html"""<h1><center> Mesh </h1> </center>"""

# ╔═╡ 00c6d677-e7a1-48e8-bb40-c0d883daf8f5
begin
	coordinates_min = (-1.0, -1.0) # minimum coordinates (min(x), min(y))
	coordinates_max = (1.0, 1.0) # maximum coordinates (max(x), max(y))
...
# ╔═╡ 3687904f-8116-4132-8bf1-0fb8ed65da3c
html"""<h1> <center> Collect spatial discretization data structures </center> </h1>"""
...
# ╔═╡ fcab4707-675b-400b-adec-c79f00144e7f
html"""<h1><center>Callbacks </h1></center>"""

# ╔═╡ ed7c660c-797d-4eab-be92-17b9574ced4a
begin
...
# ╔═╡ c7a55335-7213-4b32-8b63-3ae17daf9115
html"""<h1><center> Run the simulation </h1></center> """

# ╔═╡ 5dce294d-e8b0-4261-b246-14697b9de960
sol = solve(ode, CarpenterKennedy2N54(williamson_condition = false), dt = 1.0,
	        save_everystep = false, callback = callbacks);

# ╔═╡ 97c8cf0c-5dde-4155-825c-9efdad2c5b35
html"""<h1> <center> Plot the solution </center> </h1> """

# ╔═╡ 303e803a-ab62-454a-a1ce-4a3ac9f6e820
plot(sol)

# ╔═╡ 7c550081-a420-4f4f-b168-04b57f607d00
html"""
<h1 class="custom-heading"><center> Trixi.jl and me: CFD Simulations</h1>
"""

# ╔═╡ 386945f4-e4cb-480f-89e0-a6d6a2c9c49c
html"""
<center>
<img width = 500 src="https://gist.githubusercontent.com/Arpit-Babbar/6f8e445d9126f41aa96c9e2b5fb0bc99/raw/a9b0413923eb65fe66b1f3180c8029648302d2db/naca0012.png">
<img width = 500 src="https://gist.github.com/Arpit-Babbar/6f8e445d9126f41aa96c9e2b5fb0bc99/raw/a9b0413923eb65fe66b1f3180c8029648302d2db/von_karmann.png">
</center>
"""

# ╔═╡ 764cab33-25e9-46ca-b2d0-52f0b67c18d1
md"""
...
html"""
<h1 class="custom-heading"><center> Trixi.jl and me: KH Cloud simulations</h1>
"""

# ╔═╡ 51be0600-45cc-4675-b248-9c3064f9d093
html"""<center>
<font size = "4">
...
# ╔═╡ 0bde656d-9276-4e5e-aff2-ac5a8d1dd002
html"""<h1><center>Initial conditions: Some constants </h1></center> """

# ╔═╡ 0ce02b6f-ecfb-4c14-adf8-39f8cfc443ec
begin
	...
	function f_profile(z)
	  ...
	  z1, z2, z3 = 7000.0, 10000.0, 11500.0
	  if z <= z1
	      return bv21 * z
	  elseif z <= z2
	      return (bv21 - bv22) * z1 + bv22 * z
	  elseif z <= z3
	      return (bv21 - bv22) * z1 + (bv22 - bv23) * z2 + bv23 * z
	  else
	      return (bv21 - bv22)*z1 + (bv22 - bv23)*z2 + (bv23 - bv24)*z3 + bv24*z
	  end
	end
	
	t_profile(z) = (t00() + 10.0) * exp(f_profile(z) / g() - kappa() * z / hp())
	
	p_profile(z) = p00() * exp(-z / hp())
	
	rho_profile(z) = p_profile(z) / (rd() * t_profile(z))
end;

# ╔═╡ c4b7f877-6f1c-4d2f-a496-1d9178817cb0
html"""<h1><center>Initial conditions </h1></center> """

# ╔═╡ 52e2d468-5fe0-46ab-b249-fd6b3a70e50b
function initial_condition_kelvin_helmholtz_instability(x, t, equations)
  y1, y2 = -0.5, 0.5 # Interface locations
  rho = rho_profile(x[2])
  p = p_profile(x[2])
  a = 0.05
  v1 = tanh((x[2] - y1)/a) - tanh((x[2] - y2)/a) - 1.0
  w0 = 0.1
  sigma = 0.05 / sqrt(2.0)
  v2 = w0 * sin(4.0 * pi * x[1]) # Perturbation term
  exp_term = exp(-(x[2] - 0.5)^2 / (2.0 * sigma^2))
  exp_term += exp(-(x[2] + 0.5)^2 / (2.0 * sigma^2))
  v2 *= exp_term
  # Tracer variable
  xi = 0.5 * (tanh((x[2] - y2)/a) - tanh((x[2] - y1)/a) + 2.0)
  return prim2cons((rho, v1, v2, p, xi), equations)
end

# ╔═╡ c78727de-8f4f-4a8a-865e-a8e87ff4cbc1
html"""<h1><center>Equations </h1></center> """

# ╔═╡ f2d2c006-4815-4ab1-ad46-f9e1ea77428f
begin
	gamma = 1.4
	flow_equations = CompressibleEulerEquations2D(gamma)
	tracer_equations = PassiveTracerEquations(flow_equations, Val(1))
end

# ╔═╡ 098b1352-7757-41ba-a422-cf5737e93fa3
html"""<h1><center>DG solver </h1></center> """

# ╔═╡ 732c670f-a2aa-4ac9-9209-3e036d2047f3
begin
    surface_flux = flux_lax_friedrichs
    volume_flux = FluxTracerEquationsCentral(flux_ranocha)
    dg_solver = DGSEM(polydeg = 3, surface_flux = surface_flux,
...
begin
    ...
    coords_max = (+1.0, +1.0)
	initial_refinement_level = 5
    mesh_cart = TreeMesh(coords_min, coords_max;
...
begin
	...
	@inline function source_terms_gravity(u, x, t, equations)
	  rho, _, rho_v2, _ = u
	  return SVector(zero(eltype(u)), zero(eltype(u)), -g() * rho, 
		  		     -g() * rho_v2, 0.0)
	end
end

# ╔═╡ 461d1423-a785-40da-9492-c08f2a9317d5
html"""<h1><center>Semidiscretize into an ODE </h1></center> """

# ╔═╡ 8b9c7b78-c047-45e7-8e4a-9ed0628d04a4
begin
	semi_kh = SemidiscretizationHyperbolic(mesh_cart, tracer_equations,
		initial_condition_kelvin_helmholtz_instability, dg_solver,
	    boundary_conditions = boundary_conditions,
        source_terms = source_terms_gravity)
	
	tspan = (0.0, 1.0)
	ode_kh = semidiscretize(semi_kh, tspan)
end;

# ╔═╡ c5e96cf0-b425-48f1-9818-18974e8f6e87
html"""<h1><center> Simulation with a movie </h1></center> """

# ╔═╡ eae9f00e-2a4c-4fc6-bb8f-144ca63a24a3
begin
	    tstops = range(tspan..., length = 101)
	    integrator = init(ode_kh,
	                      SSPRK43(); controller = PIDController(0.55, -0.27, 0.05),
	                      abstol = 1.0e-5, reltol = 1.0e-5,
	                      maxiters = 1.0e7,
	                      save_everystep = false)
	
	    fig_rho = plot(; title = "density", clims = (1.2302, 1.2304))
	    fig_xi1 = plot(; title = "tracer 1", clims = (-1.0, 1.0))
	
	    anim = Plots.Animation()
	    for _ in TimeChoiceIterator(integrator, tstops)
	        (; t, u) = integrator
	        pd = PlotData2D(u, semi_kh; 
							solution_variables = cons2density_and_tracers)
	
	        kwargs = (xguide = "", yguide = "", ticks = false, fontsizes()...,
	                  size = (600, 500), aspect_ratio = :equal, 
					  margin = 0.6 * Plots.cm,)
	        plot!(fig_rho, pd["rho"]; title = L"density $\rho$", kwargs...)
	        plot!(fig_xi1, pd["xi_1"]; title = L"tracer $\xi_1$", kwargs...)
	        fig = plot(fig_rho, fig_xi1;
	                   layout = (1, 2), size = (1800, 500),
	                   margin = 0.0 * Plots.cm,
	                   aspect_ratio = :equal)
	        Plots.frame(anim, fig)
	    end
	    Plots.mp4(anim, joinpath(@__DIR__, "khi_tracer.mp4"); fps = 10)
end

# ╔═╡ 6b950f4b-6c39-4dc6-a15f-669eb57820bc
html"""
<h1 class="custom-heading"><center>Thank you!</center></h1>
"""
...
# ╔═╡ 3b56f251-4e81-42dc-8990-318ab9769758
space
...
manifest_format = "2.0"
project_hash = "fd93303380deb22347c0b1d097771bb7ccf8ab8b"
