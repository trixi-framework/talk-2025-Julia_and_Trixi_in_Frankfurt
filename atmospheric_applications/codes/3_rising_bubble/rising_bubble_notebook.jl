...
struct WarmBubbleSetup
    ...
    potential_temperature_ref::Float64  # constant background potential temperature steady state
	R::Float64     
    function WarmBubbleSetup(; g = 9.81, c_p = 1004.0, c_v = 717.0, gamma = c_p / c_v, p_0 = 100_000.0, potential_temperature_ref = 300.0)
        R = c_p - c_v
		new(g, c_p, c_v, gamma, p_0, potential_temperature_ref, R)
    end
end
...
function initial_condition_rising_bubble(x, t, equations::CompressibleEulerEquations2D)
	@unpack g, c_p, c_v, R, p_0, potential_temperature_ref = setup
...
equations = CompressibleEulerEquations2D(setup.gamma)
...
semi = SemidiscretizationHyperbolic(mesh, equations, initial_condition_rising_bubble, solver,
